...
import com.databricks.spark.xml.{MRv1XmlInputFormat, XmlInputFormat, XmlOptions}

private[xml] object MRv1XmlFile {
  val DEFAULT_INDENT = "    "
  ...
  def withCharset(
    ...
    if (Charset.forName(charset) == Charset.forName(XmlOptions.DEFAULT_CHARSET)) {
      context.hadoopFile[LongWritable, Text, MRv1XmlInputFormat](location).map(
        pair => new String(pair._2.getBytes, 0, pair._2.getLength))
    } else {
      context.hadoopFile[LongWritable, Text, MRv1XmlInputFormat](location).map(
  ...
  def saveAsXmlFile(
    ...
    val indent = MRv1XmlFile.DEFAULT_INDENT
    val rowSeparator = MRv1XmlFile.DEFAULT_ROW_SEPARATOR
